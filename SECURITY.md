...
- Credentials can be kept out of the MCP client's environment with `UNIFI_<SERVER>_PASSWORD_FILE` or `UNIFI_<SERVER>_PASSWORD_COMMAND`; these indirections are honoured only from the environment the server was started with, never from a `.env` file in the working directory
