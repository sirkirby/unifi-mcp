...
from unifi_core.network.managers.connection_manager import ConnectionManager, controller_error_code
...
class ClientManager:
    ...
    async def get_client_details(self, client_mac: str, *, existence_only: bool = False) -> Any:
        ...
        if user_record is None and path_mac is not None and not (existence_only and active_record is not None):
            try:
                user_record = await self._get_user_by_mac(path_mac)
            except Exception as e:
                if isinstance(e, ResponseError) and "404" in str(e):
                    # aiounifi raises ResponseError for HTTP 404 (and 429): a 404
                    # means the controller does not serve /stat/user, which is
                    # no answer, so the lists' verdict stands.
