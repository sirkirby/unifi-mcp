...
**Mutating tools require `confirm=true` before dispatch resolution:** `apps/api/src/unifi_api/services/actions.py:dispatch_action` classifies each tool via `_classify_action()`, which requires BOTH `entry.permission_action in {create, update, delete}` AND `entry.read_only_hint is False` to agree before treating it as a `"mutation"` (raises `ValueError` on disagreement — fail-closed). If `action_kind == "mutation"` and `confirm` is not `true`, dispatch raises `ValueError(f"tool '{tool_name}' requires confirm=true")`. There is no `CONFIRM_REQUIRED_TOOLS` allowlist — it was removed in favor of this two-signal (`permission_action` + `readOnlyHint`) classification. This manifest-driven gate is independent of any confirm logic the tool layer itself implements.
