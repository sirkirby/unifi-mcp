...
def test_snapshot_action_shapes_reference_or_inline_image() -> None:
    ...
    assert Snapshot.from_manager_output({"snapshot_url": "protect://cameras/camera-1/snapshot"}).to_dict() == {
        "snapshot_url": "protect://cameras/camera-1/snapshot"
    }
    assert Snapshot.from_manager_output({"image_base64": "anBlZw==", "content_type": "image/jpeg"}).to_dict() == {
        "image_base64": "anBlZw==",
