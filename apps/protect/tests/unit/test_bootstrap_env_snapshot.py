...
This probes the real import order in a subprocess.
"""
...
def test_dotenv_supplied_command_is_refused_and_never_runs(tmp_path):
    ...
    (tmp_path / ".env").write_text(
        f"UNIFI_PROTECT_PASSWORD_COMMAND=touch {marker}\nUNIFI_PROTECT_HOST=10.9.9.9\nUNIFI_PROTECT_USERNAME=x\n",
        encoding="utf-8",
    )
    probe = tmp_path / "probe.py"
    probe.write_text("import unifi_protect_mcp.bootstrap as b\nb.load_config()\n", encoding="utf-8")
    ...
    assert "UNIFI_PROTECT_PASSWORD_COMMAND" in result.stderr
