...
| Variable | Value |
|----------|-------|
| `UNIFI_ACCESS_PASSWORD_FILE` | Path to a file whose contents are the password (Docker and systemd secrets convention). Trailing newlines are dropped. |
| `UNIFI_ACCESS_PASSWORD_COMMAND` | A command whose stdout is the password, for example `security find-generic-password -a alice -s unifi-mcp -w` or `pass show unifi/admin`. |

The command is split like a shell would (`shlex`) but is not run through a shell, so `$VAR`, pipes and redirects are passed to the program literally, and backslashes are escape characters (on Windows write paths with forward slashes). Its stdin is closed, so a helper that prompts fails immediately instead of reading the MCP stream. Set exactly one spelling per level: `UNIFI_ACCESS_PASSWORD` next to `UNIFI_ACCESS_PASSWORD_FILE` refuses to start as ambiguous. A missing, empty, multi-line or oversized file, a non-zero exit, or empty or multi-line output also refuse to start (exit code 6) with the variable name in the log; stdout of a failing command is never logged, and only the tail of its stderr is. `UNIFI_ACCESS_API_KEY_FILE` and `UNIFI_ACCESS_API_KEY_COMMAND` behave the same way.
