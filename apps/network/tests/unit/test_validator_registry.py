...
TOOLS_DIR = pathlib.Path("apps/network/src/unifi_network_mcp/tools")
KEY_RE = re.compile(
    r'UniFiValidatorRegistry\.(?:validate|validate_and_apply_defaults)\(\s*[\"\']([a-z_]+)[\"\']'
)
...
def _all_referenced_keys() -> set[str]:
    ...
    keys |= {"firewall_policy_create", "firewall_policy_v2_create", "firewall_policy_v2_update"}
    return keys
