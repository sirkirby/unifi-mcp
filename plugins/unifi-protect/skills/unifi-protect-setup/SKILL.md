...
the Claude target `set-env.sh` only adds keys, so remove any previously saved plain
`UNIFI_PROTECT_PASSWORD` entry before switching. The Codex and OpenClaw targets
