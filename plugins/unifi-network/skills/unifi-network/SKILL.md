...
All tools return: `{"success": true, "data": ...}`, `{"success": false, "error": "..."}`, or `{"success": true, "requires_confirmation": true, "preview": ...}`. Always check `success` first. Network/WLAN and VPN-state writes are read back after execution and also report `mutation_applied`, `partial_success`, `persisted_fields`, `dropped_fields`, `coerced_fields`, and actual post-write details. A failed confirmed write is not necessarily a rollback; inspect those fields before retrying or compensating.
